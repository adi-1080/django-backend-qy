            ...
            try:
                if date == 'Current':
                    ...
                    try:
                        ...
                        if trailing_pe > 0 and growth_rate and growth_rate != 0:
                            peg_ratio = trailing_pe / (growth_rate * 100)
                            statistics["PEG_Ratio"][date] = f"{peg_ratio:.2f}"
                        else:
                        ...
                        if closest_quarter_idx.size > 0:
                            ...
                            if not quarterly_balance_sheet.empty:
                                ...
                                if 'EBITDA' in quarterly_financials.index:
                                    ttm_ebitda = sum(quarterly_financials.loc['EBITDA'][quarterly_financials.columns <= closest_quarter][-4:])
                                    if ttm_ebitda > 0:
                                        ev_to_ebitda = enterprise_value / ttm_ebitda
                                        statistics["Enterprise_Value_to_EBITDA"][date] = f"{ev_to_ebitda:.2f}"
                                    else:
                                        statistics["Enterprise_Value_to_EBITDA"][date] = "--"
                                else:
                                    statistics["Enterprise_Value_to_EBITDA"][date] = "--"
                                ...
                                try:
                                    ...
                                    if forward_eps and forward_eps > 0:
                                        forward_pe = historical_price / forward_eps
                                        statistics["Forward_PE"][date] = f"{forward_pe:.2f}"
                                    else:
                                        statistics["Forward_PE"][date] = "--"
                                except:
                                    statistics["Forward_PE"][date] = "--"
                                ...
                                # For PEG Ratio
                                try:
                                    trailing_pe = float(statistics["Trailing_PE"][date].replace('--', '0'))
                                    growth_rate = info.get('earningsGrowth', 0)

                                    if trailing_pe > 0 and growth_rate and growth_rate != 0:
                                        peg_ratio = trailing_pe / (growth_rate * 100)  # Convert growth rate to percentage
                                        statistics["PEG_Ratio"][date] = f"{peg_ratio:.2f}"
                                    else:
                                        statistics["PEG_Ratio"][date] = "--"
                                except:
